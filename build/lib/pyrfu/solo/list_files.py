...
from .solo_config import CONFIG
...
def list_files(tint=None, var=None):
    ...
    #      solo_level_instrument[-datatype]-rate_YYYYMMDD_version.cdf

    file_name = "solo" + "_" + var["lev"].upper() + "_" + var["inst"] + "(-)?.*" + "-" + var[
        "tmmode"] + "_([0-9]{8})_V(\d+)(\d+).cdf"
    ...
    for date in days:
        local_dir = os.sep.join(
            [data_path, var["inst"], var["tmmode"], level_and_dtype, date.strftime("%Y"),
             date.strftime("%m")])
    ...
    file_name = "solo.*_([0-9]})_V(\d+)(\d+).cdf"
