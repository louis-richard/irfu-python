...
def sliding_derivative(time_series, t_units: str = "ns", window_size=3):
    ...
    at each point is estimated using the first and last points in the window (central difference).
    ...
    derivative : xarray.DataArray
        ...
        The length of this array matches the length of `time_series`. For points near the 
        boundaries (where a full window cannot be formed), the result will be NaN.
    
    Notes:
    ------
    - The derivative is approximated using central differences for points that can accommodate 
      the window size. For edge points, the output will contain NaN.
    - The function assumes `time_steps` are evenly spaced but works for irregular time steps 
      as well by calculating the actual time difference between the start and end of the window.
