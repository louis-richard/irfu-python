...
def fk_power_spectrum(pc=-1, tint=None, v6=None, b_xyz=None, z_phase=None, cav=128, field=True, num_k=101, num_f=200, df=None):
    """Calculates the frequency-wave number power spectrum using MMS's four spin plane probes. Follows the same
    ...
    tint : list of str
        Time interval over which the power spectrum is calculated. ``b_xyz`` should be closely aligned with one probe
        pair over this time.
    ...
    field : bool, optional
        Set to True to use electric fields calculated from opposing probes and sc_pot. Set to False to use only
        opposing probe potentials. Default is True.
    ...
    Notes
    -----
    Timing corrections are applied to ``v6`` in this function. Do not apply them before running this function.
    Directions and speeds are consistent with expectations based on time delays. Work still in progress. Time
    corrections for the probes need to be revised.
