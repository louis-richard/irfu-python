...
# -*- coding: utf-8 -*-
"""
get_pitchangle_dist.py

@author : Louis RICHARD
"""


def get_pitchangle_dist(vdf=None, b_xyz=None, tint=None, **kwargs):
    """
    Compute pitch angle distributions from burst mode data

    Parameters :
        vdf : Dataset
            Electron or ion particle distribution.

        b_xyz : DataArray
            Time series of the background magnetic field.

        tint : str/list of str/datetime
            Time or time interval. If a single element paddist is an array of the pitch-angle distribution at the time
            closest to tint. If tint has two elements paddist is a Tseries over tint.

    Options :
        angles : int/list/array
            Flag to set number of pitch angles used (single value Default is 12) or pitch angle edges (array).
            When counts are high (plasma is dense) 24 pitch angles for electrons may be used.
    ...
    Examples :
        ...
        >>> paddist, tint = mms.get_pitchangledist(vdf, b_xyz, [tint])
        >>> paddist, tint = mms.get_pitchangledist(vdf, b_xyz, [tint], angles=24)
        >>> paddist, tint = mms.get_pitchangledist(vdf, b_xyz, [tint], angles=[0, 45, 90, 135, 180])
        ...
        if isa(varargin{tmpnargin-1}, 'char')
            if strcmp(varargin{tmpnargin-1}, 'angles')
                if (length(varargin{tmpnargin}) == 1)
                    ...
                    nangles = floor(nangles); % Makesure input is integer
                    dangle = 180 / nangles;
    ...
    return paddist, theta, energy, tint
    """
    return