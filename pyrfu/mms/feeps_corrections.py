...
def feeps_corrections(feeps_alle):
    ...
    feeps_alle = deepcopy(feeps_alle)
    feeps_alle_clean = []
    
    for ic in range(len(feeps_alle)):

        feeps_alle[ic] = feeps_correct_energies(feeps_alle[ic])
        feeps_alle[ic] = feeps_flat_field_corrections(feeps_alle[ic])
        feeps_alle[ic] = feeps_remove_bad_data(feeps_alle[ic])

        split_int_ch = feeps_split_integral_ch(feeps_alle[ic])
        feeps_alle_clean = split_int_ch[0]; #dpf_feeps_alle_500kev = split_int_ch[1];
        
        feeps_alle_clean.append(feeps_remove_sun(feeps_alle_clean))
