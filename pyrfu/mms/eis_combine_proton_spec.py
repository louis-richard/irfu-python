...
def eis_combine_proton_spec(phxtof_allt, extof_allt):
    ...
    for scope in scopes_phxtof:
        ...
        n_en = idx_phxtof.size + idx_phxtof_cross.size + idx_extof.size

        comb_en, comb_en_low, comb_en_hig = [np.zeros(n_en) for _ in range(3)]

        comb_array = np.zeros((nt_phxtof, n_en))
        comb_array[:, 0:idx_phxtof.size] = phxtof_data[:, idx_phxtof]
        comb_en[0:idx_phxtof.size] = en_phxtof[idx_phxtof]
        comb_en_low[0:idx_phxtof.size] = comb_en[0:idx_phxtof.size] - \
                                         dm_phxtof[idx_phxtof]
        comb_en_hig[0:idx_phxtof.size] = comb_en[0:idx_phxtof.size] + \
                                         dp_phxtof[idx_phxtof]

        for i, i_phx, i_ex in zip(range(len(idx_phxtof_cross)),
                                  idx_phxtof_cross, idx_extof_cross):
            idx_ = idx_phxtof.size + i
