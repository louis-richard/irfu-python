...
def calc_moms(nt, arguments):
	...
	"""
	if len(arguments) > 13:
		[is_brst_data, flag_same_e, flag_de, step_table, energy0, delta_v0, energy1, delta_v1, qe, sc_pot, p_mass,
		 flag_inner_electron, w_inner_electron, phi_tr, theta_k, int_energies, vdf, delta_ang] = arguments
	else:
		[is_brst_data, flag_same_e, flag_de, energy, delta_v, qe, sc_pot, p_mass, flag_inner_electron, w_inner_electron,
		 phi_tr, theta_k, int_energies, vdf, delta_ang] = arguments
	...
	v = np.real(np.sqrt(2 * qe * (energy - sc_pot.data[nt]) / p_mass))
	v[energy - sc_pot.data[nt] - flag_inner_electron * w_inner_electron < 0] = 0
	...
	psd2_v_x_mat = -np.dot(np.cos(phi_j * np.pi / 180), np.sin(theta_k * np.pi / 180) * np.sin(theta_k * np.pi / 180))
	psd2_v_y_mat = -np.dot(np.sin(phi_j * np.pi / 180), np.sin(theta_k * np.pi / 180) * np.sin(theta_k * np.pi / 180))
	psd2_v_z_mat = -np.dot(np.ones(phi_j.shape), np.sin(theta_k * np.pi / 180) * np.cos(theta_k * np.pi / 180))
	...
	psd_mf_zz_mat = np.dot(np.ones(phi_j.shape), np.sin(theta_k * np.pi / 180) * np.cos(theta_k * np.pi / 180) ** 2)
	psd_mf_xy_mat = np.dot(np.cos(phi_j * np.pi / 180) * np.sin(phi_j * np.pi / 180), np.sin(theta_k * np.pi / 180) ** 3)
	psd_mf_xz_mat = np.dot(np.cos(phi_j * np.pi / 180), np.cos(theta_k * np.pi / 180) * np.sin(theta_k * np.pi / 180) ** 2)
	psd_mf_yz_mat = np.dot(np.sin(phi_j * np.pi / 180), np.cos(theta_k * np.pi / 180) * np.sin(theta_k * np.pi / 180) ** 2)

	for ii in int_energies:
		tmp = np.squeeze(vdf[nt, ii, :, :])
		# n_psd_tmp1 = tmp .* psd2_n_mat * v(ii)^2 * delta_v(ii) * delta_ang;
		# n_psd_e32_phi_theta(nt, ii, :, :) = n_psd_tmp1;
		# n_psd_e32(nt, ii) = n_psd_tmp

		# number density
		n_psd_tmp = delta_v[ii] * delta_ang * np.nansum(np.nansum(tmp * psd2_n_mat, axis=0), axis=0) * v[ii] ** 2
		n_psd += n_psd_tmp

		# Bulk velocity
		v_temp_x = delta_v[ii] * delta_ang * np.nansum(np.nansum(tmp * psd2_v_x_mat, axis=0), axis=0) * v[ii] ** 3
		v_temp_y = delta_v[ii] * delta_ang * np.nansum(np.nansum(tmp * psd2_v_y_mat, axis=0), axis=0) * v[ii] ** 3
		v_temp_z = delta_v[ii] * delta_ang * np.nansum(np.nansum(tmp * psd2_v_z_mat, axis=0), axis=0) * v[ii] ** 3
		...
		# Pressure tensor
		p_temp_xx = delta_v[ii] * delta_ang * np.nansum(np.nansum(tmp * psd_mf_xx_mat, axis=0), axis=0) * v[ii] ** 4
		p_temp_xy = delta_v[ii] * delta_ang * np.nansum(np.nansum(tmp * psd_mf_xy_mat, axis=0), axis=0) * v[ii] ** 4
		p_temp_xz = delta_v[ii] * delta_ang * np.nansum(np.nansum(tmp * psd_mf_xz_mat, axis=0), axis=0) * v[ii] ** 4
		p_temp_yy = delta_v[ii] * delta_ang * np.nansum(np.nansum(tmp * psd_mf_yy_mat, axis=0), axis=0) * v[ii] ** 4
		p_temp_yz = delta_v[ii] * delta_ang * np.nansum(np.nansum(tmp * psd_mf_yz_mat, axis=0), axis=0) * v[ii] ** 4
		p_temp_zz = delta_v[ii] * delta_ang * np.nansum(np.nansum(tmp * psd_mf_zz_mat, axis=0), axis=0) * v[ii] ** 4
...
def psd_moments(vdf=None, sc_pot=None, **kwargs):
	...
	**kwargs : dict
	...
	if "energy_range" in kwargs:
		if isinstance(kwargs["energy_range"], (list, np.ndarray)) and len(kwargs["energy_range"]) == 2:
			if not is_brst_data:
				energy0 = energy
	...
	if "partial_moments" in kwargs:
		...
		if partial_moments.shape == vdf.data.shape:
			sum_ones = np.sum(np.sum(np.sum(np.sum(partial_moments, axis=-1), axis=-1), axis=-1), axis=-1)
			sum_zeros = np.sum(np.sum(np.sum(np.sum(-partial_moments + 1, axis=-1), axis=-1), axis=-1), axis=-1)
			...
			if (sum_ones + sum_zeros) == vdf.data.size:
				...
				print("notice : All values are not ones and zeros in partial_moments. Full moments will be calculated")
		else:
	...
	if is_brst_data:  # Burst mode energy/speed widths
		if flag_same_e and flag_de:
			...
			v_upper = np.sqrt(2 * qe * energy_upper / p_mass)
			v_lower = np.sqrt(2 * qe * energy_lower / p_mass)
			...
			v0upper = np.sqrt(2 * qe * energy0upper / p_mass)
			v0lower = np.sqrt(2 * qe * energy0lower / p_mass)
			v1upper = np.sqrt(2 * qe * energy1upper / p_mass)
			v1lower = np.sqrt(2 * qe * energy1lower / p_mass)
	...
	args brst : 
	(is_brst_data, flag_same_e, flag_de, step_table, energy0, delta_v0, energy1, delta_v1, qe, sc_pot.data, p_mass, 
	flag_inner_electron, w_inner_electron, phi_tr.data, theta_k, int_energies, vdf.data.data, delta_ang)
	
	args fast :
	(is_brst_data, flag_same_e, flag_de, energy, delta_v, qe, sc_pot.data, p_mass, flag_inner_electron, w_inner_electron, 
	...
	if is_brst_data:
		...
		arguments = (is_brst_data, flag_same_e, flag_de, energy, delta_v, qe, sc_pot.data, p_mass, flag_inner_electron,
					 w_inner_electron, phi_tr.data, theta_k, int_energies, vdf.data, delta_ang)
